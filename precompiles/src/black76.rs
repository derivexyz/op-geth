...
pub extern "C" fn __precompile_black76(
    ...
    let ret = unsafe { slice::from_raw_parts_mut(ret_val, 48) };
...
fn write_result(call_price: [u8; 16], put_price: [u8; 16], call_delta: [u8; 16], ret: &mut [u8]) {
    // let cp = u128::from_be_bytes(call_price);
    // let pp = u128::from_be_bytes(put_price);
    // let cd = u128::from_be_bytes(call_delta);
    // println!("CALL PRICE: {} | PUT PRICE: {} | CALL DELTA: {}", cp, pp, cd);

    let mut res = [0; 48];
    res[..16].copy_from_slice(&call_price);
    res[16..32].copy_from_slice(&put_price);
    res[32..].copy_from_slice(&call_delta);
